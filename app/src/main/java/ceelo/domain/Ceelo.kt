...
fun List<Int>.compareThrows(secondPlayerThrow: List<Int>)
    ...
    whichCase.run whichCase@{
        ...
        secondPlayerThrow.whichCase.run otherWhichCase@{
            return when {
                this@whichCase > this@otherWhichCase -> DiceThrowResult.WIN
                this@whichCase < this@otherWhichCase -> DiceThrowResult.LOOSE
                else -> onSameCase(
...
fun List<Int>.onSameCase(
    ...
    AUTOMATIC_WIN_456_CASE -> DiceThrowResult.RETHROW
    AUTOMATIC_LOOSE_123_CASE -> DiceThrowResult.RETHROW
    STRAIGHT_234_345_CASE -> when {
        containsAll(`2_3_4`) && secondPlayerThrow.containsAll(`2_3_4`) -> DiceThrowResult.RETHROW
        containsAll(`3_4_5`) && secondPlayerThrow.containsAll(`3_4_5`) -> DiceThrowResult.RETHROW
        containsAll(`2_3_4`) && secondPlayerThrow.containsAll(`3_4_5`) -> DiceThrowResult.LOOSE
        else -> DiceThrowResult.WIN
    }

    UNIFORM_TRIPLET_CASE -> when {
        uniformTripletValue > secondPlayerThrow.uniformTripletValue -> DiceThrowResult.WIN
        uniformTripletValue < secondPlayerThrow.uniformTripletValue -> DiceThrowResult.LOOSE
        else -> DiceThrowResult.RETHROW
    }

    UNIFORM_DOUBLET_CASE -> when {
        uniformDoubletValue > secondPlayerThrow.uniformDoubletValue -> DiceThrowResult.WIN
        uniformDoubletValue < secondPlayerThrow.uniformDoubletValue -> DiceThrowResult.LOOSE
        else -> DiceThrowResult.RETHROW
    }

    else -> when {
        sum() > secondPlayerThrow.sum() -> DiceThrowResult.WIN
        sum() < secondPlayerThrow.sum() -> DiceThrowResult.LOOSE
        else -> DiceThrowResult.RETHROW
    }
}
